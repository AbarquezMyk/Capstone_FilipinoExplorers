...
const fullStoryData = [ {
  ...
  },
  {
    ...
    "question": "Ano ang aral sa kwento ni Ana?",
    "choices": [
      "Ang prutas ay masarap",
      "Ang tubig ay mahalaga",
      "Ang tiyaga ay may magandang bunga",
      "Ang araw ay mainit"
    ],
    "correctAnswer": 2,
    "hint": "Araw-araw niyang dinilig ang kanyang tanim."
...
const PaaralanQuest = () => {
  ...
  const [answerResults, setAnswerResults] = useState([]); // stores "correct" | "wrong" | null
  ...
  useEffect(() => {
    if (answeredQuestions.length > 0 && answeredQuestions.every(q => q)) {
  ...
  useEffect(() => {
    if (sessionFinished) {
      axios.post("http://localhost:8080/api/paaralan-quest/score/submit", {
        ...
        console.log("✅ Score and name submitted successfully.");
      }).catch((err) => {
  ...
  if (storyData.length > 0) {
    setAnsweredQuestions(Array(storyData.length).fill(false));
    setAnswerResults(Array(storyData.length).fill(null)); // initially no answers yet
  }
}, [storyData]);
...
}, [timerActive, currentIndex, answeredQuestions]);
...
const handlePrev = () => {
  ...
  if (!answeredQuestions[currentIndex]) {
    ...
    setScore(prev => prev + (isCorrect ? (usedHint ? 1 : 2) : 0));
  ...
  <div style={{
          ...
          <div style={{ flex: 1, paddingLeft: '20px' }}>
            ...
            {current.choices.map((choice, index) => (
              <button
                ...
                style={{
                  backgroundColor: selectedChoice === index ? '#d1e7dd' : '#fff',
                  marginBottom: '8px',
                  padding: '10px',
                  borderRadius: '8px',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', height: '600px', width: '220px' }}>
      ...
      <div style={{ backgroundColor: '#8B4513', padding: '20px', borderRadius: '10px', display: 'flex', flexWrap: 'wrap', gap: '10px', justifyContent: 'center' }}>
        {storyData.map((_, i) => {
          let bgColor = '#f5e5c0'; // default color for unanswered

          if (i === currentIndex) {
            bgColor = '#FFD700'; // yellow for current
          } else if (answerResults[i] === 'correct') {
            bgColor = 'lightgreen'; // green if answered correctly
          } else if (answerResults[i] === 'wrong') {
            bgColor = '#ff9999'; // red if answered incorrectly
          }
